...
const Login = () => {
  ...
  return (
    <form onSubmit={onSubmitHandller} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <input onChange={(e) => setEmail(e.target.value)} value={email} className='w-full px-3 py-2 border border-gray-800' type='email' placeholder='Email address' required/>
      ...
      <button className='bg-black text-white font-light px-8 py-2 mt-4 cursor-pointer'>{currentState === 'Sign Up' ? "Sign Up" : "Login"}</button>
    </form>
  )
}
